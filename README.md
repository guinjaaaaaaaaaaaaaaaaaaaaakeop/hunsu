...
| `check` | manifest vs this machine. `error`: missing/wrong-version plugin, engine below floor, runtime a hook needs is not on PATH, a skill name provided by two plugins with no resolution. `warn`: unpublished source, duplicate hook registration, user-level hook not in the manifest (one line per script however many events it is on, with the file it runs on this machine), `hunsu.local.json` not ignored by git (an `error` when it is committed). `info`: links, extra plugins, machine-bound paths. Exit 1 on errors |
...
- `settings` — the team's switches for its products (e.g. `{"chongdae": {"outside-runs": ["content/"]}}`). A product reads its own from this file, so a switch works as soon as it is written — also while a product's working source is tried with `dev`, when `lock` refuses. hunsu does not read them; `lock` copies them into the lock as the record of what was declared then.
- `dev-settings` — what a trial changes, declared by the project the way a package declares dev dependencies (e.g. `{"chongdae": {"commit-records": false}}`: a working source's records are written, not committed). hunsu names no product; `dev` applies these as this machine's overlay and `dev --off` takes them back.
- `hunsu.local.json` `settings` — this machine's overlay over `settings` (never committed, like `.env.local` over `.env`): a product reads its own from hunsu.json and lets this machine's override it.

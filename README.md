...
Each is also a slash command: `/hunsu:survey`, `/hunsu:init`, `/hunsu:add <plugin>`, `/hunsu:check`, `/hunsu:lock`, `/hunsu:compose`. The `compose` skill is the same flow driven by the agent.
...
| `add <plugin>` | record version and source from the survey. Local marketplace → `unpublished` + link |
...
1. `judge request --out DIR` — stage 1: names + descriptions of the locked skills and the standing modes → the judge groups them by the **situation** they claim (`cluster-response.json`).
2. `judge request --out DIR --groups DIR/cluster-response.json` — stage 2: one packet per group with the members' full SKILL.md text → findings with **quotes from each member**, a reason, and a proposed resolution.
...
```json
{
  ...
  "plugins": {"hacheong": {"version": "1.0.0", "source": "github:guinjaaaaaaaaaaaaaaaaaaaaakeop/hacheong"}},
...
- `resolutions` — keyed by situation. Shorthand for a same-name overlap: `"retro": "dakdol"` or `"deny"`. Full form, for a judged situation:
  ...
  `deny` and `use`-on-a-name reach the lock's `denied` list, which the PreToolUse hook enforces. `order` and `accept` are facts for the agent, not enforced.
...
- `roles` — role → `plugin:skill`, `session`, or an argv (a command with `{plugin:NAME}`, `{base}` …) — who provides a role in this project; runners read it from the lock and name no product themselves. Checked: a named skill must be locked.
